...
def eval_sequence(
    ...
    if not image_paths:
        return None, None, None, 0.0, f"no images in {image_dir}", None, None
    ...
    if not os.path.isfile(tracks_path):
        return None, None, None, 0.0, "no tracks.npy", None, None
...
def _render_frame(image_path, fi, gt_tracks_orig, gt_vis_mask,
    ...
    panels = []
    for tag, color, label in _PANEL_DEFS:
...
def visualize_sequence(
    ...
    """
    Save a track-dot comparison video: one panel per model, dots only.
    Output: <vis_dir>/<split_name>/<seq_name>.mp4
    """
    ...
    try:
        ...
        if _encode_video(tmp_dir, out_path, fps):
            print(f"    [viz tracks] -> {out_path}  ({S} frames @ {fps}fps)")
    finally:
...
def visualize_planes(
    ...
    """
    Save a plane-quad comparison video: one panel per model, quads only.
    GT plane = green quad, predicted plane = tag-colored quad.
    Output: <vis_dir>/<split_name>/<seq_name>_planes.mp4
    ...
    out_dir = os.path.join(vis_dir, split_name)
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, f"{seq_name}_planes.mp4")
    ...
    try:
        for fi in range(S):
            ...
            cv2.imwrite(os.path.join(tmp_dir, f"{fi:06d}.jpg"),
                        frame_rgb[:, :, ::-1], [cv2.IMWRITE_JPEG_QUALITY, 95])
        if _encode_video(tmp_dir, out_path, fps):
            print(f"    [viz planes] -> {out_path}  ({S} frames @ {fps}fps)")
    finally:
        shutil.rmtree(tmp_dir, ignore_errors=True)
...
def eval_split(
    ...
    for seq in sequences:
        ...
        gt_frame_indices_seq, gt_corners_seq = load_gt_corners(
            os.path.join(dataset_dir, seq)
        )
        gt_frame_lookup = (
...
def main():
    ...
    def discover_sequences(dataset_dir):
        """Return sorted list of subdirs that have images/ and tracks.npy."""
        seqs = []
        for name in sorted(os.listdir(dataset_dir)):
            seq_dir = os.path.join(dataset_dir, name)
            if (
                os.path.isdir(seq_dir)
                and os.path.isdir(os.path.join(seq_dir, "images"))
